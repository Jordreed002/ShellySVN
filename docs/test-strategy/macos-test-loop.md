...
- [x] **P6** — `local-status-server.macos.test.ts`: forced-darwin `getDefaultLocalStatusSocketPath` — `.sock` under userData, deterministic, distinct, embeds raw path (contrast to Windows' hashed pipe). Mirrors the sibling forced-win32 describe (`local-status-server.ts:31-38`).
- [x] **P7** — `pathResolution.macos.test.ts`: forced-darwin forward-slash + case-preserving output for POSIX working-copy roots (`pathResolution.ts:11-18`). NOTE: the case-normalization core (`svn-cache-service`/`status-service` win32 lowercase) was already covered by the Windows agent's `test(win)` commits, which also added a forced-darwin POSIX case-sensitivity describe — so no duplication; this fills the renderer path-resolution macOS gap.
- [x] **P8** — `index.macos.test.ts`: `window-all-closed` macOS boundary — stays alive on darwin, quits on win32/linux (`index.ts:225-229`). Keeps `app.whenReady` pending + explicit factory mocks of every local import so the heavy ready body never runs. NOTE: the `activate` (dock-click recreate) handler and the hidden titleBar/`trafficLightPosition` window chrome live inside the ready body and remain uncovered (would need resolving whenReady with fuller window mocks or a small extraction).
- [x] **P9** — `tests/e2e/macos-integrations.spec.ts` (`test.skip(process.platform !== 'darwin')`): real-launch integration — main process runs on darwin, `app:getPlatform` IPC reaches the renderer as 'darwin', and a visible BrowserWindow with non-zero bounds is created. Complements the unit suites with a packaged-main-process launch (macOS CI E2E runner is macos-14).
