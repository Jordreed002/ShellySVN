...
export function RepoBrowserContent() {
  ...
  const isValidUrl = repoUrl && (repoUrl.startsWith('http://') || repoUrl.startsWith('https://') || repoUrl.startsWith('svn://') || repoUrl.startsWith('svn+ssh://'))
  ...
  const { data: directoryData, isLoading, refetch } = useQuery({
    queryKey: ['repo-browser', currentUrl, credentials],
    queryFn: async () => {
      const result = await window.api.svn.list(currentUrl, 'HEAD', 'immediates', credentials || undefined)
      return result
    },
  ...
  const handleBrowse = useCallback((node: RepoNode) => {
    if (node.kind === 'dir') {
  ...
  const handleConnect = useCallback(async () => {
    ...
    try {
      ...
      if (storedCreds) {
        creds = { username: storedCreds.username, password: storedCreds.password }
      }
    } catch {
      setCredentials(null)
    }
                ...
                <table className="w-full">
                  <thead className="sticky top-0 bg-bg-secondary border-b border-border">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-right text-xs font-medium text-text-muted uppercase tracking-wider w-20">Size</th>
                      <th className="px-4 py-2 text-right text-xs font-medium text-text-muted uppercase tracking-wider w-20">Revision</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase tracking-wider w-32">Author</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-text-muted uppercase tracking-wider w-40">Date</th>
                    </tr>
                  </thead>
              ...
              <div className="w-72 bg-bg-secondary border-l border-border p-4">
                ...
                <div className="space-y-3 text-sm">
                  <div>
                    ...
                    <span className="ml-2 text-text">{selectedNode.kind === 'dir' ? 'Directory' : 'File'}</span>
                  </div>
                  <div>
                ...
                <div className="mt-6 space-y-2">
                  ...
                  <button
                    type="button"
                    onClick={() => navigate({ to: '/history', search: { path: selectedNode.url } })}
      ...
      {!isValidUrl && (
        <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
          ...
          <p className="text-sm text-text-secondary max-w-sm mb-6">
            Enter a repository URL above to browse files and folders directly on the server,
            without needing a working copy.
          </p>
