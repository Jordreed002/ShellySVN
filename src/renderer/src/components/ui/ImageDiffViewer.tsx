    ...
    if (isOpen && filePath) {
      ...
      const loadImage = async (path: string, _revision?: string): Promise<string> => {
        try {
          ...
          throw new Error(`Failed to load image: ${(err as Error).message}`, { cause: err });
        }
      };
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          ...
          {!isLoading && !error && newImage && (
            <div
              ...
              style={{
                cursor: viewMode === 'swipe' ? 'col-resize' : isPanning ? 'grabbing' : 'default',
              }}
            >
              ...
              {viewMode === 'overlay' && (
                <div className="relative w-full h-full flex items-center justify-center">
                  <div className="relative max-w-full max-h-full" onMouseDown={handlePanMouseDown}>
                    {/* Base image */}
                    <img
                      src={newImage}
                      alt="Modified"
