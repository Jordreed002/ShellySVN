...
import type { LazyTreeNode, LazyTreeLoaderState, SvnListResult, AuthCredential } from '@shared/types'
...
export function useLazyTreeLoader(rootUrl: string, credentials?: AuthCredential) {
  ...
  const { 
    ...
    queryFn: async () => {
      try {
        ...
        throw new Error(`Failed to load root tree: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    },
    enabled: !!rootUrl,
  ...
  const loadNodeMutation = useMutation({
    ...
    onError: (error, variables) => {
      // Update error state
      setNodeErrors(prev => new Map(prev).set(variables.path, error instanceof Error ? error.message : 'Unknown error'))
   ...
   */
  const loadNode = useCallback(async (path: string, nodeCredentials?: AuthCredential) => {
   ...
   */
  const isNodeLoading = useCallback((path: string) => {
   ...
   */
  const getNodeError = useCallback((path: string) => {
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: async () => {
          const result = await window.api.svn.list(
            ...
            undefined,
            'immediates',
            nodeCredentials ? { username: nodeCredentials.username, password: nodeCredentials.password } : undefined
          )
          return {
            path,
            nodes: svnListToLazyTreeNode(result),
