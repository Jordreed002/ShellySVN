...
export function useActivityFeed() {
  ...
  const addActivity = useCallback(
    async (activity: Omit<ActivityItem, 'id' | 'timestamp' | 'isRead'>): Promise<ActivityItem> => {
   ...
   * Add commit activity
   */
  const addCommitActivity = useCallback(
    async (logEntry: SvnLogEntry, repositoryPath: string): Promise<ActivityItem> => {
      return addActivity({
        type: 'commit',
  ...
  const markAsRead = useCallback(
    async (id: string): Promise<void> => {
      const newActivities = activities.map((a) => (a.id === id ? { ...a, isRead: true } : a));
      setActivities(newActivities);
      await saveActivities(newActivities);
    },
    [activities, saveActivities]
