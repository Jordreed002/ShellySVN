...
export function useCommitMessageHistory() {
  ...
  const addMessage = useCallback(
    async (message: string, path?: string) => {
  ...
  const searchMessages = useCallback(
    (query: string) => {
      ...
      return history.filter((h) => h.message.toLowerCase().includes(lowerQuery));
    },
    [history]
  );
    ...
    const loadTemplates = async () => {
      try {
        ...
        if (stored && Array.isArray(stored) && stored.length > 0) {
          ...
          setTemplates([
            ...DEFAULT_TEMPLATES,
            ...stored.filter((t) => !DEFAULT_TEMPLATES.some((d) => d.id === t.id)),
          ]);
        }
      } catch (err) {
  ...
  const addTemplate = useCallback(async (template: Omit<CommitTemplate, 'id'>) => {
    ...
    setTemplates((prev) => {
      ...
      window.api.store.set(
        TEMPLATES_KEY,
        newTemplates.filter((t) => t.id.startsWith('custom-'))
      );
      return newTemplates;
    });
