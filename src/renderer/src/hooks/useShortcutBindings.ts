...
export function useShortcutBindings() {
  ...
  const setBindingEnabled = useCallback(
    async (id: string, enabled: boolean): Promise<void> => {
      const newBindings = bindings.map((b) => (b.id === id ? { ...b, enabled } : b));
  ...
  const getByCategory = useCallback(
    (categoryId: string): ShortcutBinding[] => {
  ...
  const importBindings = useCallback(
    async (json: string): Promise<void> => {
      try {
        ...
        throw new Error('Invalid bindings format', { cause: error });
      }
    },
    [bindings, saveBindings]
  );
