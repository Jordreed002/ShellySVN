...
interface OperationQueueState {
  ...
  // Actions
  addOperation: (
    op: Omit<Operation, 'id' | 'status' | 'retryCount' | 'startedAt' | 'completedAt'>
  ) => string;
...
export const useOperationQueue = create<OperationQueueState>((set, get) => ({
  ...
  cancelOperation: (id) => {
    ...
    set((state) => ({
      operations: state.operations.map((o) =>
        o.id === id ? { ...o, status: 'cancelled' as const, completedAt: Date.now() } : o
      ),
    }));
  },
  ...
  getOperationByPath: (path, type) => {
    return get().operations.find((op) => op.path === path && (!type || op.type === type));
  },
}));
