...
export function useWorkspaceLayouts() {
  ...
  const createLayout = useCallback(
    async (name: string, description?: string): Promise<WorkspaceLayout> => {
      const now = Date.now();
  ...
  const updateLayout = useCallback(
    async (id: string, updates: Partial<WorkspaceLayout>): Promise<void> => {
  ...
  const updatePanel = useCallback(
    async (panelId: string, updates: Partial<PanelConfig>): Promise<void> => {
  ...
  const duplicateLayout = useCallback(
    async (id: string): Promise<WorkspaceLayout> => {
