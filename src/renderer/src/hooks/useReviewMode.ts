...
export function useReviewMode(repositoryPath: string) {
  ...
  const updateBundle = useCallback(
    async (bundleId: string, updates: Partial<ReviewBundle>): Promise<void> => {
      const newBundles = bundles.map((b) =>
        b.id === bundleId ? { ...b, ...updates, updatedAt: Date.now() } : b
      );
      setBundles(newBundles);
      await saveBundles(newBundles);
  ...
  const addToBundle = useCallback(
    async (bundleId: string, entry: SvnStatusEntry): Promise<void> => {
  ...
  const removeFromBundle = useCallback(
    async (bundleId: string, path: string): Promise<void> => {
      const bundle = bundles.find((b) => b.id === bundleId);
      if (!bundle) return;
  ...
  const updateReviewStatus = useCallback(
    async (bundleId: string, path: string, status: ReviewItem['reviewStatus']): Promise<void> => {
      const bundle = bundles.find((b) => b.id === bundleId);
      if (!bundle) return;

      await updateBundle(bundleId, {
        items: bundle.items.map((i) => (i.path === path ? { ...i, reviewStatus: status } : i)),
      });
    },
    [bundles, updateBundle]
  );
  ...
  const addComment = useCallback(
    async (bundleId: string, path: string, content: string, lineNumber?: number): Promise<void> => {
      const bundle = bundles.find((b) => b.id === bundleId);
      if (!bundle) return;
      ...
      await updateBundle(bundleId, {
        items: bundle.items.map((i) =>
          i.path === path ? { ...i, comments: [...i.comments, comment] } : i
        ),
      });
    },
    [bundles, updateBundle]
  ...
  const exportAsPatch = useCallback(
    async (bundleId: string): Promise<string> => {
      const bundle = bundles.find((b) => b.id === bundleId);
  ...
  const getStats = useCallback(() => {
    ...
    return {
      ...
      changesRequested: activeBundle.items.filter((i) => i.reviewStatus === 'changes-requested')
        .length,
