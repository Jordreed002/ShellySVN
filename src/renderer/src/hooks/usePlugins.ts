...
export function usePlugins() {
    ...
    return {
      ...
      ui: {
        ...
        refreshUI: () => {
          ...
          window.dispatchEvent(
            new CustomEvent('shelly:refresh', {
  ...
  const installPlugin = useCallback(
    async (manifest: PluginManifest): Promise<PluginInstance> => {
  ...
  const setPluginEnabled = useCallback(
    async (pluginId: string, enabled: boolean): Promise<void> => {
      const newPlugins = plugins.map((p) => (p.manifest.id === pluginId ? { ...p, enabled } : p));
      setPlugins(newPlugins);
      await savePlugins(newPlugins);
    },
    [plugins, savePlugins]
  ...
  const registerHook = useCallback(
    (pluginId: string, hook: string, handler: PluginHookHandler): void => {
  ...
  const executeHook = useCallback(
    async (hook: string, data: unknown): Promise<unknown[]> => {
  ...
  const getCommands = useCallback((): PluginCommand[] => {
    ...
    for (const plugin of plugins) {
      ...
      if (plugin.manifest.contributes?.commands) {
        commands.push(
          ...plugin.manifest.contributes.commands.map((cmd) => ({
            ...cmd,
