...
export function useCodeReviewIntegration() {
  ...
  const generateUnifiedDiff = useCallback(
    async (paths: string[], _options: Partial<ReviewExportOptions> = {}): Promise<string> => {
      const diffs: string[] = [];
   ...
   */
  const generateReviewBoardFormat = useCallback(
    async (paths: string[], options: Partial<ReviewExportOptions> = {}): Promise<string> => {
   ...
   */
  const generatePhabricatorFormat = useCallback(
    async (paths: string[], options: Partial<ReviewExportOptions> = {}): Promise<string> => {
      const diff = await generateUnifiedDiff(paths, options);
   ...
   */
  const generateGitHubFormat = useCallback(
    async (paths: string[], logEntry?: SvnLogEntry): Promise<string> => {
   ...
   */
  const generateGitLabFormat = useCallback(
    async (paths: string[], logEntry?: SvnLogEntry): Promise<string> => {
   ...
   */
  const exportForReview = useCallback(
    async (
      paths: string[],
      format: ReviewExportFormat,
