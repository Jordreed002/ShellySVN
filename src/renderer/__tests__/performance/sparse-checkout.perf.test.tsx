...
function generateMockTreeRoots(itemCount: number, depth = 2): LazyTreeNode[] {
  ...
  for (let i = 0; i < itemCount; i++) {
    ...
    const node: LazyTreeNode = {
      ...
      children: hasChildren && depth > 0 ? generateMockTreeChildren(`/item-${i}`, 3, depth - 1) : []
    }
 ...
 */
function generateMockTreeChildren(parentPath: string, count: number, depth: number): LazyTreeNode[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    children.push({
      ...
      isLoaded: depth <= 0 || !hasChildren,
      hasChildren: hasChildren && isDir,
      children: hasChildren ? generateMockTreeChildren(`${parentPath}/child-${i}`, 2, depth - 1) : []
    })
  }
...
describe('Sparse Checkout Performance Tests', () => {
  describe('Initial Render Performance', () => {
    ...
    it('should render 50,000 items within performance target', async () => {
      ...
      console.log(`[PERF] Initial render (${VERY_LARGE_REPO_SIZE} items): ${durationMs.toFixed(2)}ms`)
  ...
  describe('Virtualization Verification', () => {
    it('should only render visible items (not all 10,000)', async () => {
      ...
      console.log(`[PERF] Rendered DOM nodes for ${LARGE_REPO_SIZE} items: ${renderedItems.length}`)
    ...
    it('should maintain consistent render time as item count increases', async () => {
      ...
      console.log(`[PERF] Item count ratio: ${ratio100To10k}x, Time ratio: ${timeRatio.toFixed(2)}x`)
  ...
  describe('Memory Usage Tests', () => {
    it('should not leak memory on repeated renders', async () => {
      ...
      if (initialMemory > 0 && finalMemory > 0) {
        ...
        console.log(`[PERF] Memory increase after 10 re-renders: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`)
