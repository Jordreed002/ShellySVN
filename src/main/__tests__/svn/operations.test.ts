...
describe('MockSvnExecutor', () => {
  ...
  describe('basic functionality', () => {
    ...
    it('should throw on non-zero exit code', async () => {
      ...
      await expect(mock.execute(['status', '--xml', '/path'])).rejects.toThrow('Authentication failed')
    })
  })
      ...
      const diff = MockSvnResponses.diff([
        {
          ...
          newPath: 'b/file.ts',
          hunks: [
            { oldStart: 1, newStart: 1, content: '+new line' }
          ]
        }
      ])
...
describe('SVN Operations with MockSvnExecutor', () => {
  ...
  describe('checkout operation', () => {
    ...
    it('should handle sparse paths', async () => {
      await mock.execute(['checkout', '--depth', 'empty', 'https://repo.url', '/path', 'src', 'docs'])
  ...
  describe('property operations', () => {
    it('should list properties', async () => {
      mock.setResponse('proplist', {
        stdout: '<?xml version="1.0"?><properties><property name="svn:ignore">*.o</property></properties>'
      })
  ...
  describe('import operation', () => {
    it('should import path to URL', async () => {
      ...
      await mock.execute(['import', '-m', 'Import message', '/local/path', 'https://repo.url/trunk'])
