...
export type MockSvnHandler = (
  args: string[],
  cwd?: string
) => MockSvnResponse | Promise<MockSvnResponse>;
...
export class MockSvnExecutor {
  ...
  async execute(
    args: string[],
    cwd?: string,
    _context?: Partial<SvnExecutionContext>
  ): Promise<string> {
...
+new line
 ...
 line3`;
  ...
  wasCalled(command: string): boolean {
    return this.callHistory.some((call) => call.args.includes(command) || call.args[0] === command);
  }
  ...
  getCallCount(command: string): number {
    return this.callHistory.filter(
      (call) => call.args.includes(command) || call.args[0] === command
    ).length;
  }
...
export const MockSvnResponses = {
   ...
   */
  status(
    entries: Array<{ path: string; status: string; revision?: number; author?: string }>
  ): string {
    const entriesXml = entries
      .map((e) => {
        ...
        const commitXml = hasCommitInfo
          ? `<commit revision="${e.revision || 0}">
...
<info>
   ...
   */
  log(
    entries: Array<{ revision: number; author?: string; date?: string; message?: string }>
  ): string {
    const entriesXml = entries
      .map(
        (e) => `
...
<lists>
  ...
  diff(
    ...
    return files
      .map((f) => {
        const hunksContent = f.hunks
          .map((h) => `@@ -${h.oldStart},1 +${h.newStart},1 @@\n${h.content}`)
          .join('\n');
...
${hunksContent}`;
   ...
   */
  blame(
    lines: Array<{ lineNumber: number; revision: number; author?: string; content: string }>
  ): string {
