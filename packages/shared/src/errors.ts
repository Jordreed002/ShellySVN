...
export function parseSvnError(stderr: string, operation: string): AppError {
  ...
  if (lower.includes('is not a working copy')) {
    return {
      code: ErrorCode.SVN_NOT_WORKING_COPY,
      message: ERROR_MESSAGES[ErrorCode.SVN_NOT_WORKING_COPY],
    };
  }
  ...
  if (lower.includes('conflict')) {
    return {
      ...
      message: ERROR_MESSAGES[ErrorCode.SVN_CONFLICT],
      details: { operation },
    };
  }
  if (lower.includes('network') || lower.includes('connection timed out')) {
    return {
      code: ErrorCode.SVN_NETWORK_ERROR,
      message: ERROR_MESSAGES[ErrorCode.SVN_NETWORK_ERROR],
    };
  }
