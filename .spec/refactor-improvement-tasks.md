...
- [ ] Move `src/shared` into `packages/shared` or remove `src/shared/package.json`.
...
- [ ] Replace duplicated logic-engine SVN types with imports from the shared package.
...
- [ ] Define a typed IPC channel contract in shared code.
...
- [ ] Move file status derivation and cache invalidation helpers out of `FileExplorer.tsx`.
...
- [ ] Split `SettingsDialog.tsx` into tab panels.
...
- [ ] Fix `RepoBrowserEnhanced` mount initialization.
...
- [ ] Expand coverage include patterns for renderer feature logic.
  - File: `vitest.config.ts`.
  - Acceptance: extracted hooks/helpers from Phase 3 are included in coverage thresholds.

- [ ] Raise coverage thresholds after module extraction.
...
- [ ] Align README architecture with the real runtime path.
...
- [ ] Document the intended module ownership rules.
