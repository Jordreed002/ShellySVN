...
| --- | --- | --- | --- | --- |
| File manager integration | Windows Explorer context menu commands | complete in app contract, release-gated by native helper artifact | `WINDOWS_CONTEXT_MENU_COMMANDS` covers checkout, update, commit, diff, log, revert, cleanup, resolve, lock/unlock, branch/tag, switch, merge, and properties; shell tests cover the command contract | Ship signed helper and run packaged smoke tests on Windows release artifact |
| File manager integration | Windows Explorer status overlays | complete in app contract, release-gated by native helper artifact | `OVERLAY_STATUS_MAP` covers SVN text statuses; `FILE_MANAGER_STATUS_PRESENTATION` adds locked item presentation from SVN lock metadata; shell tests cover locked presentation | Ship signed helper and verify registration/overlay behavior on Windows release artifact |
| File manager integration | macOS Finder Sync context menus | complete in app contract, release-gated by signed Finder Sync extension | `FINDER_CONTEXT_MENU_COMMANDS` mirrors the common working-copy action set and shell diagnostics report missing packaged helper state | Ship signed Finder Sync extension and verify registration on macOS x64/arm64 release artifacts |
| File manager integration | macOS Finder badges | complete in app contract, release-gated by signed Finder Sync extension | `FINDER_BADGE_STATUS_MAP` covers Finder-supported status badges; `FILE_MANAGER_STATUS_PRESENTATION` defines locked badge handoff where Finder allows it | Ship signed Finder Sync extension and verify badge permissions on macOS x64/arm64 release artifacts |
| File manager integration | Diagnostics and repair actions | complete in app contract, release-gated by packaged native helper verification | `ShellIntegrationManager.getStatus()`, `ShellIntegrationDialog`, and package verification tests expose helper presence, registration state, limitations, and repair actions | Keep release blocker checklist open until clean-machine signed artifact validation passes |
